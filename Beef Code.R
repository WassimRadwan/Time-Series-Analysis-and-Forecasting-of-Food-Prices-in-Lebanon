...
df = North_Zgharta_Zgharta_Meat_beef_canned_200_G_LBP$usdprice
...
autoplot(train)+
  ggtitle("Price of Canned Meat in Lebanon ($/200 grams)") + xlab("Year") +
  ylab("$/KG") #From 2013 to around 2018, the price has been relatively stable. However, we observe a an 
#increasing trend starting from 2019 followed by a decrease shortly after, followed by a slight increase, then 
#finally a sharp decrease. Thus, a changing trend is observed. The data shows no signs of seasonality
...
#data is non-stationary since the value of the test-statistic (1.0312) is greater than the 1% critical 
...
autoplot(transformed_train)+
  ggtitle("Price of Canned Beef in Lebanon ($/200 grams)") + xlab("Year") +
  ylab("$/KG")

#KPSS test to check whether the data is stationary or not
summary(ur.kpss(transformed_train)) 
#data is non-stationary since the value of the test-statistic (1.2523) is greater than the 1% critical 
#value (0.739). Hence, we must perform differencing
...
summary(ur.kpss(train_diff)) #the test statistic is equal to 0.1165 and it is less 
...
fit9transform= auto.arima(train, approximation=F, stepwise=F, lambda = lam) #ARIMA(0,1,0)(0,0,1)[12] 
fit9transform
...
all_models_arima$models[which.min(all_models_arima$aics)] #ARIMA (0,1,0)(0,0,1)[12]
...
#Working with the best ARIMA model, i.e. ARIMA(0,1,0)(0,0,1)[12]

fit9transform # (0,1,0)(0,0,1)[12]

summary(fit9transform)

forecasts.arima_transformed <- forecast(fit9transform, h=12) #Forecasting the next 12 months

checkresiduals(fit9transform) #The time plot shows that the variance is constant (or homoscedasticity) aside from a 
...
autoplot(forecasts.arima_transformed)+
  ggtitle("Forecasts from the ARIMA(0,1,0)(0,0,1)[12] (Transformed)") + xlab("Year") +
  ylab("") #Plotting our forecast

accuracy(forecasts.arima_transformed, test) #Checking the RMSE of the model

autoplot(fit9transform) ## checking the invertibility and stationarity conditions are satisfied. In this case, all red
...
boxcox_arima <- auto.arima(train, lambda = lam) #ARIMA(0,1,0)(0,0,1)[12]   AICc= -1.58
...
autoplot(forecasts.arima.box)+
  ggtitle("Forecasts from ARIMA(0,1,0)(0,0,1)[12] (Transformed w/o Stepwise and Approximation") + xlab("Year") +
  ylab("") #Plotting our forecast
...
fit9= auto.arima(train, trace=TRUE, test="kpss", ic="aic", 
                 approximation=F, stepwise=F) # Best model: ARIMA(0,1,0)
...
models_2 <-
  c("ARIMA(1,1,0)","ARIMA(0,1,1)","ARIMA(1,1,1)","ARIMA(1,1,2)","ARIMA(2,1,1)","ARIMA(0,1,2)",
    "ARIMA(2,1,0)","ARIMA(1,0,1)","AUTO.Arima(0,1,0)")

aics_2 <-c(AICc1, AICc2, AICc3, AICc4,AICc5,AICc6,AICc7,AICc8,AICc9)

rmses_2 <- c(rmse1_2,rmse2_2, rmse3_2,rmse4_2,rmse5_2,
             rmse6_2, rmse7_2,rmse8_2,rmse9_2)

pvalues_2 <-c(p1_2,p2_2,p3_2,p4_2,p5_2, p6_2, p7_2,p8_2,p9_2)
...
# Return the model that had the lowest AICc value:
all_models_arima_2$models_2[which.min(all_models_arima_2$aics_2)] #ARIMA(0,1,O)
...
#Working with the best ARIMA model, i.e., ARIMA(0,1,0)
...
autoplot(forecasts.arima)+
  ggtitle("Forecasts from ARIMA(0,1,0)") + xlab("Year") +
  ylab("")
...
#Auto.Arima without Stepwise and Approximation
arima_1 <- auto.arima(train) #ARIMA(0,1,0)
...
autoplot(forecasts.arima.box)+
  ggtitle("Forecasts from Auto.Arima(0,1,0)") + xlab("Year") +
  ylab("") #Plotting our forecast
...
autoplot(train) +
  ...
  xlab("Year") +
  ylab("$/200 g") +
  ggtitle("Price of Meat (Canned, Beef)") +
  guides(colour=guide_legend(title="Forecast")) #Plotting the forecasts

#Holt's (Damped) method returned the lowest (or best) AICc 
