...
#Renamed the dataset 
df = South_Saida_Saida_Rice_imported_Egyptian_KG_LBP$usdprice
...
length(data)# Length of Dataset, 12 x 9 = 108 records
...
#The Naive method has the lowest RMSE among the benchmarking methods, hence, it is the best method 
#in this case

####Plots####

autoplot(train)+
  ggtitle("Price of Rice in Lebanon ($/KG)") + xlab("Year") +
  ylab("$/KG") #From 2013 to 2019, the price has been fluctuating. However, we observe a rapidly increasing trend 
#during 2020 and onward.
...
#All the plots indicate that there is some seasonality in the time series as well as an increasing trend.
...
#data is non-stationary since the value of the test-statistic (0.9598) is greater than the 1% critical 
#value (0.739). Hence, we must perform differencing

dt = diff(train) #differencing the data 

ndiffs(dt) #we still need to difference the data one more time

dt2 = diff(dt) #Second-order differencing, i.e., differencing the data for a second time
...
#data is non-stationary since the value of the test-statistic (1.0398) is greater than the 1% critical 
#value (0.739). Hence, we must perform differencing

#Another method to check if the training set requires differencing

ndiffs(transformed_train) 

#the output was 2, indicating that we must difference the data TWICE.
...
train_diff_2 = diff(train_diff)

ndiffs(train_diff_2)

summary(ur.kpss(train_diff_2)) #the test statistic is equal to 0.0229 and it is less 
#than the all critical values. Thus, we accept H0 and conclude that the data 
...
ggtsdisplay(train_diff_2) #Based on  the ACF and PACF plots, potential ARIMA models are ARIMA(p,d,0) and ARIMA(0,d,q)
...
#fit4 returned an error in this case, so it is ignored
...
fit9transform= auto.arima(train, approximation=F, stepwise=F, lambda = lam) #ARIMA (0,2,2)
fit9transform
...
models <-
  c("ARIMA(1,1,0)","ARIMA(0,1,1)","ARIMA(1,1,1)", "ARIMA(2,1,1)","ARIMA(0,1,2)", "ARIMA(2,1,0)","AUTO.Arima(0,2,2)")

aics <-c(AICc1transform, AICc2transform, AICc3transform,
          AICc5transform, AICc6transform,
         AICc7transform,AICc9transform)

rmses <- c(rmse1,rmse2, rmse3, rmse5, rmse6, rmse7,rmse9)

pvalues <-c(p1,p2,p3,p5,p6,p7,p9) 
...
all_models_arima$models[which.min(all_models_arima$aics)] #ARIMA (0,1,1)
...
#Working with the best ARIMA model, i.e. ARIMA(0,1,1)

fit2transform # ARIMA(0,1,1)

summary(fit2transform)

forecasts.arima_transformed <- forecast(fit2transform, h=12) #Forecasting the next 12 months

checkresiduals(fit2transform) #The time plot shows that the variance is constant (or homoscedasticity) aside from a 
...
autoplot(forecasts.arima_transformed)+
  ggtitle("Forecasts from the ARIMA(0,1,1) (Transformed)") + xlab("Year") +
  ylab("") #Plotting our forecast

accuracy(forecasts.arima_transformed, test) #Checking the RMSE of the model

autoplot(fit2transform) ## checking the invertibility and stationarity conditions are satisfied. In this case, the
#red dot is within the circle, thus the fitted model is both stationary and invertible

#Auto.Arima without Stepwise and Approximation
boxcox_arima <- auto.arima(train, lambda = lam) #ARIMA(2,2,1)  AICc= -125.99
...
#be white noise (even though the ACF plot has one spike outside the bounds)

autoplot(forecasts.arima.box)+
  ggtitle("Forecasts from Auto.Arima(2,2,1) (Transformed w/o Stepwise and Approximation") + xlab("Year") +
  ylab("") #Plotting our forecast
...
#fit5 returned. Hence, it is ignored.
...
fit9= auto.arima(train, trace=TRUE, test="kpss", ic="aic", 
                 approximation=F, stepwise=F) # Best model: ARIMA (1,2,1)
confint(fit9)
...
models_2 <-
  c("ARIMA(1,1,0)","ARIMA(0,1,1)","ARIMA(1,1,1)","ARIMA(1,1,2)","ARIMA(0,1,2)","ARIMA(2,1,0)","AUTO.Arima(1,2,1)")

aics_2 <-c(AICc1, AICc2, AICc3,AICc4,AICc6,
           AICc7,AICc9)

rmses_2 <- c(rmse1_2,rmse2_2, rmse3_2,rmse4_2,
             rmse6_2, rmse7_2,rmse9_2)

pvalues_2 <-c(p1_2,p2_2,p3_2,p4_2,p6_2, p7_2,p9_2)
...
# Return the model that had the lowest AICc value:
all_models_arima_2$models_2[which.min(all_models_arima_2$aics_2)] #ARIMA(1,2,1)
...
#Working with the best ARIMA model, i.e., ARIMA(1,2,1)
...
#normally distributed, and the ACF plot displays only 1 significant autocorrelations,i.e., all spikes are within
#the blue-dashed bounds except for one. Thus, the residuals are White Noise. 

autoplot(forecasts.arima)+
  ggtitle("Forecasts from Arima(1,2,1)") + xlab("Year") +
  ylab("")
...
#Auto.Arima without Stepwise and Approximation
arima_1 <- auto.arima(train) #ARIMA(1,2,1)
...
checkresiduals(forecasts.arima_1) #Similar conclusion to the previous one

autoplot(forecasts.arima.box)+
  ggtitle("Forecasts from Auto.Arima(1,2,1)") + xlab("Year") +
  ylab("") #Plotting our forecast
...
autoplot(train) +
  ...
  xlab("Year") +
  ylab("$/KG") +
  ggtitle("Price of Rice") +
  guides(colour=guide_legend(title="Forecast")) #Plotting the forecasts

#Holt's-Winter (Multiplicative) method returned the lowest (or best) AICc 

summary(fc5)

forecasts.fc5 <- forecast(fc5, h=12)

checkresiduals(fc5) # Variance is constant in the time plot, Histogram shows signs of a 
